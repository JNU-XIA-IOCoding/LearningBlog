# PHOENIX BLOG — CLAUDE CODE EXECUTION PROMPT
# 完整全栈博客网站 · 本地自动化构建计划书
# ═══════════════════════════════════════════════════════════════
...
# 将此文件夹放入任意目录后，在该目录运行：
claude   # 启动 Claude Code，它会自动读取此 CLAUDE.md
```
...
# 0.2 创建工作目录
mkdir -p phoenix-blog/{frontend,backend,nginx,scripts}
cd phoenix-blog
...
```ini
DATABASE_URL=postgresql://phoenix:dev_password_2026@localhost:5432/phoenix_blog
JWT_SECRET=dev-phoenix-secret-change-in-production-2026
PORT=3001
...
```ini
DATABASE_URL=postgresql://USER:PASSWORD@HOST:5432/phoenix_blog
...
services:
  ...
  backend:
    ...
    env_file: ./backend/.env.development
    environment:
      DATABASE_URL: postgresql://phoenix:dev_password_2026@db:5432/phoenix_blog
    ports: ["3001:3001"]
...
# 5. 运行迁移
cd backend && DATABASE_URL="postgresql://phoenix:dev_password_2026@localhost:5432/phoenix_blog" \
...
#!/bin/bash
set -e
echo "🚀 Phoenix Blog — Production Deploy"
...
FILE="backups/phoenix_${DATE}.sql.gz"
mkdir -p backups
docker-compose exec -T db pg_dump -U phoenix phoenix_blog | gzip > "$FILE"
...
6. **admin 默认凭据**：首次运行后，`existing/backend/db/schema.sql` 中已插入 admin 用户（密码: `phoenix2026`）。提示用户首次登录后立即修改密码。
